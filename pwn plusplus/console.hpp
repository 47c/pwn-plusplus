...
namespace utilities {
	...
	inline void log( fmt::format_string<T...> fmt, T&&... args ) {
		...
		fmt::detail::is_utf8( ) ? fmt::vprint( fmt, vargs )
			: fmt::detail::vprint_mojibake( stdout, fmt, vargs );
		fmt::print( "\n" );
	}

	inline void debug_colors( ) {
		std::map<const char*, int32_t> colors = {
		...
		for ( auto color : colors ) {
			utilities::log( "{}", fmt::styled( color.first, fmt::fg( ( fmt::detail::color_type )color.second ) ) );
		}
	}
}